# 在昆仑芯 XPU 机器上使用 Paddle 和 FlagCX 指南

## 环境配置

1. 在昆仑芯 XPU 机器上准备一个 docker 容器
2. 启动 docker 容器
...
我们支持在昆仑芯 XPU 机器上使用 Paddle + FlagCX 训练 GPT3。请参考以下步骤开始
...
4. 准备训练脚本  
    ...
    ```bash
    # 这是使用 flagcx 作为通信后端在昆仑芯 XPU 机器上训练 gpt3 的脚本
    # 定义根路径
    ...
    __注意__：  
    使用昆仑芯 XPU 训练模型时，需要在模型配置 json 文件中指定设备类型：
    - `"device": "xpu"`