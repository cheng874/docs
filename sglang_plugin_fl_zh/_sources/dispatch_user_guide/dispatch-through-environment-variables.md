...
> `SGLANG_FL_FLAGOS_WHITELIST` 和 `SGLANG_FL_FLAGOS_BLACKLIST` 互斥。`SGLANG_FL_FLAGOS_WHITELIST` 优先级高于 YAML `flagos_blacklist`。
...
| `SGLANG_PLUGINS` | （全部） | SGLang 内置：筛选要加载的插件（逗号分隔） |
