...
- **Hints**：
  最顶层的编译器提示语言，专为初学者设计，提供轻量级的性能优化，无需改变程序语义或底层硬件行为。Hints 与原生 Triton 代码完全向后兼容。
- **FLIR**：
  FlagTree Linalg 中间表示，是一个多后端统一的中间层，作为将 Triton 扩展中间表示（例如 Hints、Ops 和 TLE）lowering 到硬件特定方言的中心枢纽。
