...
本模块实现了一个用于稀疏 MLA（多头注意力）机制前向传播的 Triton 内核。它演示了如何使用 `tle.load` 实现高效的内存访问和计算。
