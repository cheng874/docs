...
def get_flagrelease_models():
    ...
    while True:
        # ModelScope organization page API (unofficial API but typically stable)
        ...
        try:
            ...
            print(f"  Page {page}, found {len(models)}/{total} models total")
...
def main():
    ...
    # Group by prefix for statistics
    prefix_count = {}
    for model in models:
        # Extract part after organization name
        parts = model.split('/')
        if len(parts) > 1:
            prefix = parts[1].split('-')[0] if '-' in parts[1] else parts[1][:10]
            prefix_count[prefix] = prefix_count.get(prefix, 0) + 1
    
    print(f"  Model families: {len(prefix_count)} different families")
