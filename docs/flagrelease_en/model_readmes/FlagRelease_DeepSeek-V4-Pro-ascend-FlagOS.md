...
# Evaluation Results
## Benchmark Result
| Metrics      | DeepSeek-V4-Pro-Nvidia-Origin | DeepSeek-V4-Pro-Ascend-FlagOS |
|--------------|--------------------------------|--------------------------------------|
| GPQA | -                              | -                                    |
| Aime       | -                              | -                                 |
...
```bash
docker pull harbor.baai.ac.cn/flagrelease-public/flagrelease-ascend-deepseek-v4-pro:202604240207
```

### Download Open-source Model Weights
```bash
pip install modelscope
modelscope download --model FlagRelease/DeepSeek-V4-Pro-ascend-FlagOS --local_dir /data/DeepSeek-V4-Pro-ascend-FlagOS
```

### Start the Container
```bash
docker run   --name flagos   --network host   --ipc=host   --privileged   --device /dev/davinci1   --device /dev/davinci2   --device /dev/davinci3   --device /dev/davinci4   --device /dev/davinci5   --device /dev/davinci6   --device /dev/davinci_manager   --device /dev/devmm_svm   --device /dev/hisi_hdc   -v /usr/local/dcmi:/usr/local/dcmi   -v /usr/local/bin/npu-smi:/usr/local/bin/npu-smi   -v /usr/local/Ascend/driver/lib64/:/usr/local/Ascend/driver/lib64/   -v /usr/local/Ascend/driver/version.info:/usr/local/Ascend/driver/version.info   -v /usr/local/Ascend/driver:/usr/local/Ascend/driver   -v /usr/local/sbin:/usr/local/sbin   -v /etc/ascend_install.info:/etc/ascend_install.info   -v /data:/data   -itd harbor.baai.ac.cn/flagrelease-public/flagrelease-ascend-deepseek:202604240207 bash
docker exec -it flagos /bin/bash
```
## Service Invocation
### Invocation Script
DeepSeek-V4-Pro INT8 MP64 Four-Machine Inference Startup Instructions:
- Code path: `/data/DeepSeek-V4-Pro-ascend-FlagOS/ascend-deepseek-pro`
- Verify HCCL network connectivity
- Start command


```bash
# Verify HCCL network connectivity
for i in {0..15}; do hccn_tool -i $i -link -g; done       # All should be UP
for i in {0..15}; do hccn_tool -i $i -net_health -g; done  # All should be UP

# Start command: Execute separately on the 4 nodes, replace`MASTER` with the IP of node 0, and set `node_rank` to 0/1/2/3 in sequence:

cd /data/DeepSeek-V4-Pro-ascend-FlagOS/ascend-deepseek-pro
export HCCL_SOCKET_IFNAME=eth5200
...
torchrun \
  ...
  --master_port=29500 \
  generate.py \
    --ckpt-path /data/DeepSeek-V4-Pro-ascend-FlagOS \
    --config config_int8_mp64.json \
