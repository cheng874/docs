...
| 平台 | 设备检查命令 | 可见设备环境变量 | FlagTree 后端 | 容器镜像 |
|------|--------------|------------------|---------------|----------|
| 沐曦 MetaX | `mx-smi` | `MACA_VISIBLE_DEVICES` | `metax` | `harbor.baai.ac.cn/flagscale/megatron-lm-with-te:202603231839` |
| 海光 Hygon | `hy-smi` | `HIP_VISIBLE_DEVICES` | `hcu` | `harbor.sourcefind.cn:5443/dcu/admin/base/custom:vllm0.20.0-ubuntu22.04-dtk26.04-py3.10-MiniCPM-V-4.6` |
| 昇腾 Ascend | `npu-smi info` | `ASCEND_RT_VISIBLE_DEVICES` | `ascend` | `harbor.baai.ac.cn/flagos-dev/flagscale:manual-20260812-ascend-dev-inference` |
| 平头哥 PPU | `ppu-smi` | `CUDA_VISIBLE_DEVICES` | `ppu` | `harbor.baai.ac.cn/flagtree/flagtree-ppu-py312-torch2.10.0-sdk2.1.0-cu130-ubuntu24.04:202607-3.6-vllm0.24.0` |
...
mx-smi            # 沐曦
hy-smi            # 海光（随后执行 source /opt/dtk/env.sh）
npu-smi info      # 昇腾
ppu-smi           # 平头哥
```
...
**非 CUDA 芯片也需要设置 `CUDA_DEVICE_MAX_CONNECTIONS`** —— 通过 `te_fl.yaml` 启用多卡并行时，参数校验仍要求该变量。请在 `train_te_fl.yaml` 中补上。参见 [FlagScale issue #1289](https://github.com/flagos-ai/FlagScale/issues/1289)。

**底层切换为 FlagOS 后多卡训练报错** —— 沐曦后端的 C++ 绑定（`metax.passes.ttgpuir.add_accelerate_matmul`）与 Triton 3.6.0 的 Python API 不兼容：该绑定期望 C++ 的 `mlir::PassManager`，实际收到的是 Python 包装的 `triton._C.libtriton.ir.pass_manager`。参见 [Megatron-LM-FL issue #143](https://github.com/flagos-ai/Megatron-LM-FL/issues/143)。
...
参见 [Megatron-LM-FL issue #145](https://github.com/flagos-ai/Megatron-LM-FL/issues/145)。
...
参见 [Megatron-LM-FL issue #147](https://github.com/flagos-ai/Megatron-LM-FL/issues/147)。
...
**`FlagTune ManifestFetchError: FLAGTUNE_MANIFEST_URL is not configured`** —— FlagTune 代价模型没有可用缓存。参见 [FlagGems issue #6133](https://github.com/flagos-ai/FlagGems/issues/6133)。

**`ValueError: Invalid entry in hostfile`** —— 由 `flag_gems_log_path` 指向 hostfile 引起，见 6.3 节。参见 [FlagGems issue #6134](https://github.com/flagos-ai/FlagGems/issues/6134)。
