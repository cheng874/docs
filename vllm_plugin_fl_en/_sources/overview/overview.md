...
vllm-plugin-FL is installed from a pre-built Docker image and then integrates vLLM with the FlagOS operator, communication, and compiler stack on the target chip. See [Getting Started](../getting_started/getting-started.md) for the installation flow and [Features](features.md) for what the plugin provides.
